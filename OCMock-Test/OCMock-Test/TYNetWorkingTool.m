...
-(void)request:(RequestType)type url:(NSString *)url params:(NSDictionary *)params success:(void (^)(id responseObject))success failure:(void (^)(NSError *error))failure progress:(void (^)(NSProgress *progress))progress{
    ...
    id successBlock = ^(id responseObject) {
        [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
        success(responseObject);
    };
    id failureBlock = ^(NSError * _Nullable error) {
        [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
        failure(error);
    };
    switch (type) {
        case GET:
            [self requestWithGET:url params:params success:successBlock failure:failureBlock progress:progress];
            break;
            
         case POST:
            [self requestWithPOST:url params:params success:successBlock failure:failureBlock progress:progress];
            break;
    }
}
